...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(
            self, columns=("ID", "name", "tel", "email", "pay"), height=45, show="headings"
        )
    ...
    def records(self, name, tel, email, pay):
        self.db.insert_data(name, tel, email, pay)
        self.view_records()
    ...
    def update_records(self, name, tel, email, pay):
        self.db.cursor.execute(
            """UPDATE db SET name=?, tel=?, email=?, pay=? WHERE id=?""",
